...
I probably didn't give enough attention to the linear model first attempted. Using XGBoost almost feels like cheating. I think it would be interesting to see if I could get a linear model to work better -- maybe not as good as XGBoost, but good enough so that it is worth using it. There are a few advantages to using a simpler model: it is more easily interpretable, more portable, easier to train, and it doesn't feel like cheating. Maybe if more time was spent on the feature engineering step, I could have gotten the logistic regression to behave more reasonably. For example, it would be interesting to see if adding interaction features would increase the performance of the model. 
